...
class txtfile:
    ...
    def __init__(self):
        ...
        self.datatxt = os.path.join(stem, filename)
        txtfile.txtsaved(self.datatxt)

    @classmethod
    def txtsaved(cls, txt):
        txtfile.txtname = txt

    def Overwrite(self):
        datafile = open(self.datatxt, 'w')
        datafile.write('# test as of %s. \n' % (datetime.datetime.now()))
        datafile.close()

    def Append(self):
        datafile = open(self.datatxt, 'a')
        datafile.write('# test as of %s. \n' % (datetime.datetime.now()))
        datafile.close()
...
#                 input = Core.ProcessingControls.ProcessingWants[1]
#                 datatxt.write(str("     {}:".format(input)) + '\n')
#                 datatxt.write(self.analysisdef1)
#         datatxt.close()
